...
desc: 'Vernier calliper, screw gauge, spherometer — the three instruments every physics practical starts with. Learn to read them right.',
...
formula: '\\text{Reading} = MSR + (VSR \\times LC)',
...
'Apply: Total Reading = \\(MSR + VSR \\times LC\\)']}
},
...
formula: '\\text{Reading} = MSR + (CSR \\times LC)',
...
'Apply: Total=\\(PSR + HSR \\times 0.01\\) mm']}
},
...
'Calculate \\(R = \\frac{l^2}{6h}+ \\frac{h}{2}\\)' ]
}}]
},
...
formula: 'T = 2\\pi\\sqrt{\\frac{L}{g}} \\quad\\Rightarrow\\quad g = \\frac{4\\pi^2 L}{T^2}',
...
'Slope of L–T² graph gives \\(g = \\frac{4\\pi^2}{\\text{slope}}\\).']}
},
...
formula: 'F = kx \\quad\\Rightarrow\\quad k = \\frac{F}{x}\\ (\\text{N/m})',
lc: 'Metre scale: 1 mm',
...
formula: '\\mu_s = \\tan(\\theta) \\quad\\vert\\quad f = \\mu N = \\mu m g \\cos\\theta',
lc: 'Protractor: 1°',
steps: [ 'Place block on inclined plane.',
'Gradually increase angle θ until block starts to slide.',
'This angle is the angle of friction: \\(\\mu_s = \\tan(\\theta)\\).',
...
formula: 'Y = \\frac{FL}{A \\times \\Delta L} = \\frac{FL}{\\pi r^2 \\cdot \\Delta L}',
...
'Calculate stress = \\(\\frac{F}{A} = \\frac{F}{\\pi r^2}\\).',
'Calculate strain = \\(\\frac{\\Delta L}{L}\\).',
'\\(Y = \\dfrac{\\text{Stress}}{\\text{Strain}}\\) — plot and find slope.']}
},
...
formula: '\\eta = \\frac{2r^2(\\rho - \\sigma)g}{9v}',
...
'Apply Stokes\' formula to find \\(\\eta\\).']}
},
...
formula: 'T = \\frac{rh\\rho g}{2\\cos\\theta} \\quad (\\theta \\approx 0:\\ T = \\frac{rh\\rho g}{2})',
...
'Apply \\(T = \\frac{rh\\rho g}{2}\\) (for water in glass, contact angle ≈ 0°).']}}]
},
...
formula: 'v = 2f(L_2 - L_1) \\quad\\vert\\quad \\frac{\\lambda}{4} = L_1+e,\\ \\frac{3\\lambda}{4} = L_2+e',
lc: 'Metre scale: 1 mm',
...
'Record L, T, and calculate \\(f = \\frac{1}{2L}\\sqrt{\\frac{T}{\\mu}}\\).',
'Verify: \\(f \\propto \\frac{1}{L}\\) (constant T), \\(f \\propto \\sqrt{T}\\) (constant L).']}
},
...
formula: '\\frac{dT}{dt} = -k(T - T_0) \\quad\\Rightarrow\\quad T - T_0 = (T_i - T_0)e^{-kt}',
...
formula: '\\frac{1}{f} = \\frac{1}{v} - \\frac{1}{u}', lc: 'Optical bench: 1 mm',
...
'Calculate f from \\(\\frac{1}{f} = \\frac{1}{v} - \\frac{1}{u}\\).' ,
...
'Apply \\(n = \\dfrac{\\sin\\left(\\frac{A+D_m}{2}\\right)}{\\sin\\left(\\frac{A}{2}\\right)}\\).']}}]
},
...
formula: 'V = IR \\quad\\Rightarrow\\quad R = \\frac{V}{I} = \\text{slope of } V\\text{-}I\\text{ graph}',
...
formula: '\\frac{R}{S} = \\frac{P}{Q} \\quad\\vert\\quad S = R\\times\\frac{Q}{P} \\quad (\\text{Meter bridge: } S = \\frac{R(100-l)}{l})',
lc: 'Metre scale: 1 mm',
...
'\\(S = R \\times \\frac{100-l}{l}\\)  (meter bridge formula).'
]}}]}
};
